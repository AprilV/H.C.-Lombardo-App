"""
XGBoost Point Spread Prediction Model - FULL FEATURE SET
...
def prepare_features(df):
    ...
    y = (df['home_score'] - df['away_score']).values  # Point differential
...
def train_model(X_train, y_train, X_val, y_val):
    ...
    print_header("TRAINING XGBOOST SPREAD MODEL")
...
def evaluate_model(model, X, y, dataset_name):
    ...
    winner_correct = ((predictions > 0) == (y > 0)).sum()
    winner_acc = winner_correct / len(y)
    print(f"Winner Accuracy: {winner_acc:.1%}")
    
    return mae

def main():
    print_header("XGBoost Spread Model - Full Feature Set (46 features)")
    print(f"Training Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("Features: Basic stats + EPA + Advanced metrics + Vegas lines")
    print("Data: 2020-2025 seasons, week 2+, regular season only")
    ...
    train_mae = evaluate_model(model, X_train, y_train, "Training Set (2020-2023)")
    val_mae = evaluate_model(model, X_val, y_val, "Validation Set (2024)")
    test_mae = evaluate_model(model, X_test, y_test, "Test Set (2025)")
    ...
    model_path = 'ml/models/xgb_spread.pkl'
    features_path = 'ml/models/xgb_spread_features.txt'
    ...
    print(f"XGBoost Spread Model Complete - {len(feature_cols)} features")
