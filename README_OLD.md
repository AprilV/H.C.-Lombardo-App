...
## �️ **DATABASE INFORMATION FOR PROFESSOR FOSTER**

### **Database Location & Structure**
The application uses **SQLite database** with **live NFL data** (no mock data):

**Database File**: `nfl_betting_database/enhanced_nfl_betting.db`  
**Size**: ~106 KB (populated with real data)  
**Schema**: Enhanced structure with 7 tables + indexes + views  

### **Tables with Data (3 Total)**

| Table | Records | Description |
|-------|---------|-------------|
| **Teams** | 32 | All NFL teams with logos, conferences, divisions |
| **Games** | 14 | Week 5 game schedule from ESPN API |
| **data_collection_log** | 7 | Data collection audit trail |

### **Empty Tables (Ready for Expansion)**
- **BettingLines** (0 records) - Betting odds and spreads
- **SeasonStats** (0 records) - Season aggregate statistics  
- **TeamStats** (0 records) - Game-by-game team statistics
- **UserPredictions** (0 records) - User betting predictions
...
# View Teams table (32 NFL teams)
cursor.execute("SELECT name, abbreviation, conference, division FROM Teams LIMIT 5")
teams = cursor.fetchall()
print("Sample Teams:", teams)

# View Games table (14 Week 5 games)
cursor.execute("SELECT home_team_id, away_team_id, date, game_status FROM Games LIMIT 3")
games = cursor.fetchall()
print("Sample Games:", games)

# View collection log
cursor.execute("SELECT source, records_processed, timestamp FROM data_collection_log")
logs = cursor.fetchall()
print("Collection History:", logs)

conn.close()
```

**Method 2: Using Built-in Database Viewer**
```python
# Run the custom database viewer
python view_database.py
```

**Method 3: Through the Live Data Collector**
```python
from apis.live_data_collector import LiveNFLDataCollector

# Create collector instance
collector = LiveNFLDataCollector()
print(f"Database Path: {collector.db_path}")

# Database automatically connects to enhanced_nfl_betting.db
# Contains live ESPN API data (not mock data)
```

### **GUI Database Browser**
**DB Browser for SQLite** is installed for visual database inspection:
- **Launch**: Double-click `open_database.bat` 
- **Manual**: Open DB Browser → File → Open → Select `enhanced_nfl_betting.db`
- **View Data**: Browse Data tab → Select table from dropdown

### **Data Sources**
- **Teams Data**: ESPN API (all 32 NFL teams)
- **Games Data**: ESPN API (current week schedule)  
- **Collection Method**: Live API calls (no mock/static data)
- **Update Frequency**: Real-time via data collector
