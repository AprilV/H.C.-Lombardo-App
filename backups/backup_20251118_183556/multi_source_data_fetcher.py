...
class MultiSourceDataFetcher:
    ...
    def fetch_espn_standings(self) -> dict:
        ...
        print("\n[1/4] Fetching standings from ESPN API...")
        
        try:
            # Use STANDINGS endpoint to get ALL 32 teams, not just recent games
            ...
            if 'children' in data:
                for conference in data['children']:  # AFC and NFC
                    for standing_entry in conference.get('standings', {}).get('entries', []):
                        team = standing_entry.get('team', {})
                        ...
                        if team_name:
                            ...
                            stats = standing_entry.get('stats', [])
                            wins = next((s['value'] for s in stats if s['name'] == 'wins'), 0)
                            losses = next((s['value'] for s in stats if s['name'] == 'losses'), 0)
                            ties = next((s['value'] for s in stats if s['name'] == 'ties'), 0)
                            games_played = next((s['value'] for s in stats if s['name'] == 'gamesPlayed'), wins + losses + ties)
            ...
            print(f"   [OK] Got standings for {len(standings)} teams")
            return standings
    ...
    def scrape_teamrankings_ppg(self) -> dict:
        ...
        print("\n[2/4] Scraping PPG from TeamRankings.com...")
        ...
        try:
            ...
            print(f"   [OK] Got PPG for {len(ppg_data)} teams")
            return ppg_data
            
        except Exception as e:
            print(f"   [ERROR] TeamRankings PPG error: {e}")
            return {}
    
    def scrape_teamrankings_pa(self) -> dict:
        """Scrape PA from TeamRankings.com"""
        print("\n[3/4] Scraping PA from TeamRankings.com...")
        ...
        try:
            ...
            print(f"   [OK] Got PA for {len(pa_data)} teams")
            return pa_data
            
        except Exception as e:
            print(f"   [ERROR] TeamRankings PA error: {e}")
            return {}
    ...
    def merge_data_sources(self, standings, ppg_data, pa_data) -> dict:
        ...
        print(f"   [OK] {complete_teams} teams have complete data")
        if incomplete_teams:
            print(f"   [WARNING] {len(incomplete_teams)} teams missing some data:")
    ...
    def update_database(self, teams_data: dict) -> bool:
        ...
        try:
            ...
            print(f"   [OK] Updated {updated_count} teams in database")
            return True
    ...
    def run_full_update(self) -> bool:
        ...
        print("\n" + "="*70)
        if success:
            print("[SUCCESS] MULTI-SOURCE UPDATE COMPLETE")
        else:
