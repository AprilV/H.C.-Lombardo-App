@echo off
REM H.C. Lombardo NFL Analytics - DEVELOPMENT MODE
REM Hot reload enabled - changes appear instantly!

echo.
echo ==========================================================
echo   H.C. LOMBARDO - DEVELOPMENT MODE (Hot Reload)
echo ==========================================================
echo   React: Port 3000 (hot reload enabled)
echo   Flask: Port 5000 (API only)
echo   Perfect for: Active development, testing changes
echo ==========================================================
echo.
...
python -c "import psycopg2; conn = psycopg2.connect(host='localhost', port=5432, database='nfl_analytics', user='postgres', password='REDACTED_DB_PASSWORD'); cursor = conn.cursor(); cursor.execute('SELECT COUNT(*) FROM teams'); count = cursor.fetchone()[0]; conn.close(); print(f'      Database Ready: {count} teams loaded')" 2>nul
if errorlevel 1 (
