...
"""
print(f"[LOADING] api_routes_hcl.py from: {__file__}")
print("[MODULE] ★★★ REWRITTEN get_team_games function loaded ★★★")
...
def get_team_games(team_abbr):
    """Get full season schedule including future games"""
    team_abbr = team_abbr.upper()
    season = request.args.get('season', default=2025, type=int)
    ...
    cur.execute("""
        ...
        WHERE (g.home_team = %s OR g.away_team = %s) AND g.season = %s AND g.is_postseason = FALSE
        ORDER BY g.week ASC
    """, (team_abbr, team_abbr, team_abbr, team_abbr, team_abbr, team_abbr, team_abbr, season))
    
    games = cur.fetchall()
    cur.close()
    conn.close()
    
    return jsonify({'success': True, 'count': len(games), 'team': team_abbr, 'season': season, 'games': games})
