-- =============================================================================
-- H.C. LOMBARDO APP - PRODUCTION HCL SCHEMA CREATION
-- =============================================================================
-- Purpose: Create HCL (Historical Context Layer) schema for game-by-game data
-- Testing: Run in PRODUCTION database first before production deployment
...
CREATE INDEX IF NOT EXISTS idx_games_home_team ON hcl.games(home_team);
CREATE INDEX IF NOT EXISTS idx_games_away_team ON hcl.games(away_team);

COMMENT ON TABLE hcl.games IS 'Game metadata, betting lines, weather, and context for all NFL games';
COMMENT ON COLUMN hcl.games.game_id IS 'Unique identifier format: YYYY_WW_AWAY_HOME';
COMMENT ON COLUMN hcl.games.week IS 'Week number: 1-18 regular season, 19+ playoffs';
COMMENT ON COLUMN hcl.games.is_postseason IS 'TRUE for playoff games';
COMMENT ON COLUMN hcl.games.spread_line IS 'Consensus point spread (negative = home favored, e.g., -3.0)';
COMMENT ON COLUMN hcl.games.total_line IS 'Consensus over/under total points (e.g., 46.0)';
COMMENT ON COLUMN hcl.games.roof IS 'Stadium roof type: outdoors, closed, retractable, dome';
COMMENT ON COLUMN hcl.games.temp IS 'Game temperature in Fahrenheit (NULL for domed stadiums)';
COMMENT ON COLUMN hcl.games.away_rest IS 'Days of rest for away team since last game';
COMMENT ON COLUMN hcl.games.home_rest IS 'Days of rest for home team since last game';
...
CREATE TABLE IF NOT EXISTS hcl.team_game_stats (
  ...
  PRIMARY KEY (game_id, team),
  FOREIGN KEY (game_id) REFERENCES hcl.games(game_id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_tgs_team ON hcl.team_game_stats(team);
CREATE INDEX IF NOT EXISTS idx_tgs_season_week ON hcl.team_game_stats(season, week);
CREATE INDEX IF NOT EXISTS idx_tgs_season_team ON hcl.team_game_stats(season, team);

COMMENT ON TABLE hcl.team_game_stats IS 'Comprehensive performance statistics for each team in each game';
COMMENT ON COLUMN hcl.team_game_stats.is_home IS 'TRUE if team is home, FALSE if away';
COMMENT ON COLUMN hcl.team_game_stats.result IS 'Game result: W (win), L (loss), T (tie)';
...
CREATE INDEX IF NOT EXISTS idx_blines_game ON hcl.betting_lines(game_id);

COMMENT ON TABLE hcl.betting_lines IS 'Historical betting odds from sportsbooks';
COMMENT ON COLUMN hcl.betting_lines.line_type IS 'Type: spread, total, moneyline';
...
CREATE INDEX IF NOT EXISTS idx_injuries_team ON hcl.injuries(team);
CREATE INDEX IF NOT EXISTS idx_injuries_season_week ON hcl.injuries(season, week);

COMMENT ON TABLE hcl.injuries IS 'Weekly injury reports for players';
...
COMMENT ON TABLE hcl.weather IS 'Weather conditions for outdoor games';
...
CREATE MATERIALIZED VIEW IF NOT EXISTS hcl.v_game_matchup_display AS
SELECT 
  g.game_id,
  g.season,
  g.week,
...
FROM hcl.games g
LEFT JOIN hcl.team_game_stats h ON g.game_id = h.game_id AND h.is_home = TRUE
LEFT JOIN hcl.team_game_stats a ON g.game_id = a.game_id AND a.is_home = FALSE
ORDER BY g.season DESC, g.week DESC, g.game_date DESC;

CREATE UNIQUE INDEX IF NOT EXISTS idx_vgmd_game_id ON hcl.v_game_matchup_display(game_id);

COMMENT ON MATERIALIZED VIEW hcl.v_game_matchup_display IS 'Game-level view with home vs away stats pivoted side-by-side';
...
CREATE OR REPLACE FUNCTION hcl.refresh_matchup_views()
RETURNS void AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY hcl.v_game_matchup_display;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION hcl.refresh_matchup_views IS 'Refresh all materialized views after data loads';
...
-- UNION ALL
-- SELECT 'betting_lines', COUNT(*) FROM hcl.betting_lines
-- UNION ALL
-- SELECT 'injuries', COUNT(*) FROM hcl.injuries
-- UNION ALL
-- SELECT 'weather', COUNT(*) FROM hcl.weather;
...
-- =============================================================================
-- Next Steps:
-- 1. Run this SQL in PRODUCTION database (pgAdmin or psql)
-- 2. Verify tables created: SELECT table_name FROM information_schema.tables WHERE table_schema = 'hcl';
-- 3. Run historical data loader: python ingest_historical_games.py --PRODUCTION --seasons 2024
-- 4. Verify data loaded: Run verification queries above
-- 5. Refresh materialized view: SELECT hcl.refresh_matchup_views();
-- 6. If successful, migrate to production (use 'hcl' schema instead of 'hcl')
-- =============================================================================
